...
def forward_pass_tanh( message_number ,testing = False ):
	...
	first_layer_neurons = tanh(first_layer_neurons)


	second_layer_neurons = np.dot(first_layer_neurons,weight_matrix_2)
	second_layer_neurons = tanh(second_layer_neurons)


	output_layer_neurons = np.dot(second_layer_neurons,weight_matrix_3)
	predicted_output     = tanh(output_layer_neurons[0])						# This is the output obtained from the neural network after each forward pass		
...
def backward_pass_tanh( predicted_output , message_number ):
	...
	delta_3[0] = 2 * ( predicted_output - actual_output ) * tanh( predicted_output , True ) 		 # Delta values of last layer calculated



	delta_2 = np.dot(weight_matrix_3,delta_3)
	second_layer_neurons = tanh(second_layer_neurons,True)
	delta_2 = second_layer_neurons * delta_2



	delta_1 = np.dot(weight_matrix_2,delta_2)
	first_layer_neurons = tanh(first_layer_neurons,True)							# Delta values of first layer calculated
...
def create_neural_network_tanh( ):			# Neural Network with tanh function as the activation function
	...
	for iterations in range(10):
		...
		for message_number in range(train_data_lines):
			...
			predicted_output = forward_pass_tanh( message_number )
			backward_pass_tanh( predicted_output , message_number )	


	#************************************ In Sample Error Calculation ********************************#	

		for training_message in range(train_data_lines):
			output 	= forward_pass_tanh( training_message )
	...
	#************************************ Out Sample Error Calculation ********************************#
		...
		for test_message in range(test_data_lines):
			output 	= forward_pass_tanh( test_message ,True )
...
print 'Creating Neural Network Using Tanh Function...'
create_neural_network_tanh()
print 'Time taken:   ',time.time()-ticks
ticks = time.time()
